...
/** Neutral ink ramp, so a full table of scores does not turn into a wall of lime. */
export function ScoreCell({ value }: { value: number | null }) {
  ...
  return (
    <td className="border-b border-[var(--rule-soft)] px-3 py-2.5 text-right tabular-nums">
      <span
        className="inline-block min-w-12 rounded-md px-2 py-0.5"
        style={{
          background: `color-mix(in oklab, var(--foreground) ${Math.round(value * 0.16)}%, transparent)`,
        }}
      >
        {value.toFixed(1)}
      </span>
...
/**
 * Confidence meter. Color carries the band, length carries the value, and the
 * number is always printed, so nothing here is encoded by color alone.
 */
