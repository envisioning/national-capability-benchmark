...
const PARTICIPATE_NOTES: Record<string, string> = {
  ...
  [contactHref]: 'A form that sends a message to the Envisioning team.',
}

const METHOD_NOTES: Record<string, string> = {
  [methodHref]: 'Normalization, scoring, confidence and the coverage floor.',
...
export async function GET(request: Request): Promise<Response> {
  ...
  const lines = [
    ...
    '## Data',
    '',
    `- [Data package descriptor](${rawHref('data/out/datapackage.json')}): Frictionless descriptor. Version, license, sources, and every resource with its schema. Fetch this first.`,
    `- [Country index](${rawHref('data/out/index.json')}): every country, ${spell(DIMENSIONS.length)} dimensions each, with score, confidence, coverage and trend. No indicator rows. Use it for cross-country questions.`,
    `- [One country in full](${rawHref('data/out/countries/BRA.json')}): every indicator row with its raw value, its normalized value, its source, its year and its whole series. Swap BRA for any ISO3 code in the index.`,
    `- [One indicator across every country](${rawHref('data/out/indicators/economic_complexity.json')}): the same data, organized by indicator. Swap the id for any scored indicator.`,
    ...
    '## Optional',
    '',
    `- [Why this exists](${rawHref('docs/WHY.md')}): the claim under test and what would disprove it.`,
